...
| 架构 | 已验收 | 019f18f0-959a-75e0-8c8b-a47b19261fb0 | 总控/用户 | 已输出移动端 MVP 技术路线，并根据总控 QA 后决策派发开发 + UI/Frontend 小修；不写代码、不部署、不写凭据 | 等待开发 + UI/Frontend 小修回调后复核两项风险是否解除，再结合运维门禁建议是否进入发布确认 | QA 后小修监督中 |
...
| 开发 | 小修中 | 019f193a-669b-7b63-9828-496013c03bb0 | 架构/总控 | 处理 QA 后总控不接受的 restart URL 未清旧 `?result=` 问题；不改评分、题库、结果数据、免责声明、部署配置或凭据 | 完成小修、重跑测试/typecheck/build 与 Playwright 回归后回调架构并抄送总控 | QA 后小修中 |
| UI/PPT | 小修中 | 019f193a-669b-7b63-9828-496013c03bb0 | 架构/内容主编/总控 | 处理 QA 后总控不接受的结果页顶部装饰遮挡前缀/标题问题；不改产品定位、免责声明或 image2 资产 | 完成 390x844、430x932、1280x900 结果页视觉回归和分享卡无溢出回归后回调架构 | QA 后小修中 |
...
| 运维 | 已验收 | 019f1993-bf21-7da1-ab40-503844745d1b | 架构/总控 | 已完成 Cloudflare Pages 发布前预检：构建核验、配置建议、控制台步骤、部署后验证清单和回滚预案；不越权部署生产 | 等待开发 + UI/Frontend 完成 QA 后小修、用户确认生产发布，并由有权限的人推送目标 commit 到 GitHub | L3 运维预检通过，待小修回归/发布授权 |
